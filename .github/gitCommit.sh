...
if [ -z "$choice" ] || [ "$choice" == "Y" ]; then
    ...
    tikzInsertLine=$(grep -n "pdflatex -shell-escape image.tex" ~/.config/nvim/init.vim | sed 's/:.*$//g')
    sed -i 's|\[tikzInsert\]:.*$|\[tikzInsert\]: https://github.com/zhaoshenzhai/dotfiles/blob/master/nvim/init.vim#L'"$tikzInsertLine"'|g' README.md
