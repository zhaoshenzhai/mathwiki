...
Proved by: [[Heine-Borel Theorem]], [[Heine-Cantor Theorem]], [[Sequential slash limit characterizations of continuity]], [[Uniform continuity preserve Cauchyness]], [[Metric-completeness of R]], [[Sequential Limit Theorems in R]]
...
_Proof_. For the converse, suppose now that $\Omega$ is bounded and let $\bar{f}:\bar{\Omega}\to\R$ be a continuous extension of $f$. Then, by Heine-Borel, $\bar{\Omega}$ is compact, so $\bar{f}$ is uniformly continuous on $\bar{\Omega}$. But then so is any restriction of it, so $f$ is uniformly continuous on $\Omega$.
