...
* (Elementary Divisors). There exist distinct prime ideals $\gen{p_j}\nsubgrp R$ and positive integers $r_{ij}\in\N$ such that $\Tor M\iso\bigoplus_{i=1}^n\bigoplus_{j=1}^mR/\langle p_j^{r_{ij}}\rangle$.
* (Primary Decomposition). There exist distinct prime ideals $\gen{p_j}\nsubgrp R$ such that $\Tor M\iso\bigoplus_{j=1}^mM\l(p_j\r)$, where $M\l(p\r)\coloneqq\l\{m\in M\st\ex k\geq1:p^km=0\r\}$.

Each decompositions of $\Tor M$ is unique up to a permutation of the factors.

```

<i>Proof.</i> First, note that the existence and uniqueness of the three decompositions of $\Tor M$ are equivalent.
