...
<i>Proof.</i> We first prove that any linearly independent subset $S\subseteq V$ cannot contain the zero vector. Indeed, if $0\in S$, then $1\cdot0+\sum_{i=1}^{n}0\cdot s_i=0$ for every $s_1,\dots,s_n\in S$. But this is not a trivial linear combination of vectors in $\l\{0,s_1,\dots,s_n\r\}$, so $S$ is not linearly independent.
...
$$\begin{align}
    \span\l(\em\r)&=\bigcap\l\{U\subseteq\l\{0\r\}\st\em\subseteq U\land U\textrm{ is a linear subspace of }\l\{0\r\}\r\} && \textrm{Definition of linear span} \\
    &=\bigcap\l\{U\subseteq\l\{0\r\}\st U\textrm{ is a linear subspace of }\l\{0\r\}\r\} && \em\subseteq U\textrm{ vacuously} \\
