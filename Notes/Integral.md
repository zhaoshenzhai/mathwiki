...
* For a $\mu$-measurable simple function $s:X\to\R$, say $s=\sum_{i<n}a_i\chi_{A_i}$ for some $a_i\in\R$ and some $A_i\in\Meas_\mu$, set $\int s\,\d\mu\coloneqq\sum_{i<n}a_i\mu\l(A_i\r)$.
...
<b>Remark.</b> For any $A\in\Meas_\mu$ and any $f\in L^1$, let $\int_Af\,\d\mu\coloneqq\int\l(f\cdot\chi_A\r)\,\d\mu$. If $f\in L^+$, then $\mu_f:A\mapsto\int_Af\,\d\mu$ is a measure on $\Meas_\mu$. ^integral-measure-abs-cont-measure
...
<b>Remark.</b> A function $f:X\to\bar{\R}$ on a topological space with Borel measure $\mu$ is said to be <i>locally-integrable</i> if $f\cdot\chi_K\in L^1\!\l(X,\mu\r)$ for all compacts $K\subseteq X$. Denote the collection of all such functions by $L^1_\textrm{loc}\!\l(X,\mu\r)$.<span style="float:right;">$\blacklozenge$</span>
