...
* ($\axitop[2]$): Let $\mc{U}\subseteq\mc{T}_\textrm{cf}$. If $\mc{U}=\em$, then $\bigcup\mc{U}=\em\in\mc{T}_\textrm{cf}$. Otherwise, using De Morgan$\textrm{'}$s Laws, we have
$$\begin{equation}
...
* ($\axitop[3]$): Let $\mc{U}\subseteq\mc{T}_\textrm{cf}$ be non-empty and finite. Using De Morgan$\textrm{'}$s Laws again, we have
$$\begin{equation}
