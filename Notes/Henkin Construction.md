...
Let $\mc{L}$ be a language in $\FOL$. If $H$ is a Henkin $\mc{L}$-theory, then it admits a model of cardinality at-most $\l|\mc{L}\r|$.

```

<i>Proof.</i> Set $M\coloneqq\mc{C}(\mc{L})/\!\sim$, where $\sim$ is the equivalence relation defined by $c_1\sim c_2$ iff $H\proves\l(c_1=c_2\r)$; that this is an equivalence relation follows from the <i>equality-is-an-equivalence</i>, <i>generalization</i>, and <i>instantiation</i> axioms. We let $M$ be the universe of an $\mc{L}$-structure $\mc{M}$ by interpreting
$$\begin{equation}
    \begin{alignedat}{2}
