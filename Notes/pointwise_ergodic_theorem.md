...
Let $(X,\mu)$ be a probability measure space and let $T:X\to X$ be pmp. Birkhoff's _Ergodic Hypothesis_ states that the time and space averages for certain functions $f\in L^1(X,\mu)$ coincide. It turns out that this hypothesis holds iff $T$ is \ref[ergodic]{ergodic}, which we prove below following \cite{Tse22}.

# Statement and Proof

\begin{theorem}[Birkhoff's Pointwise Ergodic Theorem]\label{pointwise}

Let $(X,\mu)$ be a probability measure space and let $T:X\to X$ be pmp. If $T$ is ergodic, then for every $f\in L^1(X,\mu)$, we have $\lim_nA_n^Tf=_\mu\int f\\,\d\mu$.
