...
* <span style="color:gray">($1\Rightarrow2,3$).</span> To show that $\mc{M}$ is $\kappa$-homogeneous, let $A\subseteq M$ with $\l|A\r|<\kappa$ and suppose that $f:A\eleminto\mc{M}$. For any $a\in M\comp A$, there is some $b\in M$ realizing $\Gamma_{\!a}(x)$, so $\tp^\mc{M}(\vec{a},a)$ is contained in $\tp^\mc{M}(f(\vec{a}),b)$. Thus $f\cup\l\{(a,b)\r\}$ is partial-elementary, as desired. That $\mc{M}$ is universal follows from the Lemma with $A\coloneqq\mc{N}$.
