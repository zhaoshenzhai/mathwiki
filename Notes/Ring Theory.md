...
* In a [principal ideal domain](Principal%20Ideal%20Domain%20(PID).md), one can, via [B$\acute{\textrm{e}}$zout$\textrm{'}$s Identity](Bezout's%20Identity.md), define the [greatest common divisor](Greatest%20Common%20Divisor.md) for any two elements $r,s\in R$.
* In a [Euclidean Domain](Euclidean%20Domain.md), the greatest common divisor can be computed via the [Euclidean Algorithm](Euclidean%20Algorithm.md).<span style="float:right;">$\blacklozenge$</span>
