...
* (<i>Quotient</i>). We equip a quotient $X/\!\sim$ with the topology by declaring $U\subseteq X/\!\sim$ to be open iff $p^{-1}\!\l(U\r)\subseteq X$ is open. If $A\subseteq X$ and $\sim$ is an equivalence relation on $X$, the <i>saturation</i> of $A$ is $\Sat\l(A\r)\coloneqq\l\{x\in X\st\ex b\in B:x\sim b\r\}$. A subset $A\subseteq X$ is said to be <i>saturated</i> if $\Sat\l(A\r)=A$. ^quotient
    * A sufficient condition for the quotient $q:A\to A/\!\sim$ to be homeomorphic to $p:X\to X/\!\sim$ is that $\Sat\l(A\r)=X$ and, for all saturated open sets $U\subseteq X$, the set $\Sat\l(U\r)$ is open in $X$ too. Indeed, it follows from $\Sat\l(A\r)=X$ that the continuous natural map $\phi:A/\!\sim\,\to X/\!\sim$ is a bijection. A computation now shows that $p^{-1}\!\l(f\l(U\r)\r)=\Sat\l(q^{-1}\!\l(U\r)\r)$, so for every open set $A\subseteq A/\!\sim$, $f\l(U\r)\subseteq X/\!\sim$ is open too. Hence $f$ is a homeomorphism, as desired.
