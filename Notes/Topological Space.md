...
Constructions: [[Closure et al]], [[Sequential Limits]], [[Functional Limits]], [[Covering Space]], [[Homotopy]], [[Lift]], [[Mapping Cylinder]], [[Space of Continuous Functions]], [[Topological Subspace]]
...
* The isomorphisms in $\cattop$ are continuous bijections whose inverses is also continuous. Note that the second condition is necessary; for instance, $\exp:\l[0,1\r)\to S^1$ is a continuous bijection whose inverse is <i>not</i> continuous. A sufficient condition for a continuous bijection $f:X\to Y$ to have a continuous inverse is that $X$ is compact and $Y$ is Hausdorff, in which case every closed set $F\subseteq X$ is compact, so $f\l(F\r)$ is compact and hence closed too.
* The category $\cattop$ admits finite products, which we construct by equipping $X\times Y$ with the topology generated by the basis $\mc{B}\coloneqq\l\{U\times V\st U\in\mc{T}_X,V\in\mc{T}_Y\r\}$. Note that if $\mc{B}_X$ and $\mc{B}_Y$ are respective bases for $\mc{T}_X$ and $\mc{T}_y$, then we may take $U\in\mc{B}_X$ and $V\in\mc{B}_Y$ instead. Furthermore, the product topology also admits a subbasis consisting of all sets of the form $\pi^{-1}_X\!\l(U\r)$ and $\pi^{-1}_Y\!\l(V\r)$ for $U\in\mc{T}_X$ and $V\in\mc{T}_Y$. This makes the product topology on $X\times Y$ the weakest topology making the projections continuous.<span style="float:right;">$\blacklozenge$</span>
^product-space
