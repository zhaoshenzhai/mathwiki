...
_Let $K$ be a field and consider a subset $B\subseteq V$ of a vector space $V$ over $K$. Then $B$ is a basis of $V$ iff every non-zero vector $v\in V$ can be uniquely represented as a linear combination of vectors in $V$; formally, iff_
$$\begin{equation}
    \fa v\in V\setminus\l\{0_V\r\},\ex!n\in\N^\ast,\ex!\alpha_1,\dots,\alpha_n\in K\setminus\l\{0\r\},\ex!\l\langle s_1,\dots,s_n\r\rangle\in S^n:v=\sum_{i=1}^n\alpha_is_i.
\end{equation}$$
