...
_Proof_. We need to prove that $\Phi_\mc{B}^\mc{C}$ is a bijective linear map. To show linearity, take $T_1,T_2\in\Hom\l(V,W\r)$ and $k\in K$, so there exist unique coefficients $\alpha_{ij},\beta_{ij}\in K$ for all $i\in\l\{1,\dots,m\r\}$ and $j\in\l\{1,\dots,n\r\}$ such that
$$\begin{equation}
...
$$\begin{equation}
    \l(kT_1+T_2\r)\l(b_j\r)=kT_1\l(b_j\r)+T_2\l(b_j\r)=k\sum_{i=1}^m\alpha_{ij}c_i+\sum_{i=1}^m\beta_{ij}c_i=\sum_{i=1}^m\l(k\alpha_{ij}+\beta_{ij}\r)c_i,
\end{equation}$$
...
$$\begin{equation}
    \Phi_\mc{B}^\mc{C}\l(kT_1+T_2\r)=\l[\gamma_{ij}\r]=\l[k\alpha_{ij}+\beta_{ij}\r]=k\l[\alpha_{ij}\r]+\l[\beta_{ij}\r]=k\Phi_\mc{B}^\mc{C}\l(T_1\r)+\Phi_\mc{B}^\mc{C}\l(T_2\r).
\end{equation}$$
