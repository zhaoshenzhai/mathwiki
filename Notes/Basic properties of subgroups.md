...
* _If $H\subgrpeq N_G\!\l(K\r)$, then $HK\subgrpeq G$. In particular, if $K\nsubgrpeq G$, then $HK\subgrpeq G$ for any $H\subgrpeq G$._
...
* It suffices to show that $HK=KH$. If $hk\in HK$, then, since $h\in N_G\!\l(K\r)$, we see that $hk=k'h\in KH$ for some $k'\in K$. Similarly if $kh\in KH$.<span style="float:right;">$\blacksquare$</span>
