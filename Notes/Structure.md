...
* Two $\sigma$-structures $\mc{A}$ and $\mc{B}$ are <i>elementarily equivalent</i> if $\Th\l(\mc{A}\r)=\Th\l(\mc{B}\r)$. It follows from induction on the complexity of $\sigma$-sentences that isomorphic structures are elementarily equivalent. If $\mc{A}\substructeq\mc{B}$ is only a $\sigma$-substructure, then they agree on quantifier-free formulas, but in general for a $\sigma$-formula $\phi(\vec{v})$, the best that we can say is that $\mc{B}\models\phi(\vec{a})$ implies $\mc{A}\models\phi(\vec{a})$ whenever $\phi$ is universal, and $\mc{A}\models\phi(\vec{a})$ implies $\mc{B}\models\phi(\vec{a})$ whenever $\phi$ is existential.<span style="float:right;">$\blacklozenge$</span>
