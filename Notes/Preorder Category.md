...
_Let $\l\langle X,\leq\r\rangle$ be a preordered set. The **preorder category induced by $\l\langle X,\leq\r\rangle$** is the category_ $\cat{C}\l\langle X,\leq\r\rangle$ _whose objects are elements in $X$ and whose morphisms $f:x\to y$ consist of the pair $\l\langle x,y\r\rangle$ if $x\leq y$ and is empty otherwise. Formally,_
