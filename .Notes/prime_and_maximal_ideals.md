...
Let $\mf{a}\nsubgrp R$ be prime and suppose that $\mf{a}\subseteq\mf{b}$ for some ideal $\mf{b}\nsubgrp R$. Since $R$ is a PID, $\mf{a}=\gen{a}$ and $\mf{b}=\gen{b}$ for some $a,b\in R$, and since $\gen{a}\subseteq\gen{b}$, we have $a=bc$ for some $c\in R$. Then either $b\in\gen{a}$ or $c\in\gen{a}$; the former implies that $\mf{a}=\mf{b}$, and if $c\in\gen{a}$, then $c=ad$ for some $d\in R$, and hence $a=abd$. But then $bd=1$ since $R$ is an integral domain, so $\mf{b}=\gen{b}=R$.<span style="float:right;">$\blacksquare$</span>

{{< /env >}}

&emsp;&emsp;The question now remains whether every ring admits a maximal ideal. The answer is yes assuming $\mathsf{AC}$, and, in fact, more is true:

{{< env type="theorem" name="Krull" id="krulls_theorem" >}}

If $R\neq0$, then every proper ideal $\mf{a}\nsubgrp R$ extends to a maximal ideal $\mf{a}\subseteq\mf{m}\nsubgrp R$.
