...
_The standard topology_ $\mc{T}\l(<\r)\coloneqq\mc{T}\l(\mc{B}\r)$ _on $\mb{R}$ can be generated by the subbasis_
$$\begin{equation}
...
_Proof_. Take $x\in\mb{R}$. If $x\geq0$, observe that $x\in\l(-1,+\infty\r)$; otherwise, observe that $x\in\l(-\infty,1\r)$. It follows that $\mc{S}$ covers $\mb{R}$ and hence $\mc{S}$ is a subbasis for a topology on $X$. It remains to show that for any $B\in\mc{B}$, there exists $\mc{R}\subseteq\mc{S}$ such that $0<\l|\mc{R}\r|<\infty$ and $B=\bigcap\mc{R}$, for then the topology generated by $\mc{S}$ is $\mc{T}\l(\mc{S}\r)=\mc{T}\l(\mc{B}\r)=\mc{T}\l(<\r)$. To this end, take $B=\l(a,b\r)\in\mc{B}$ with $a,b\in\mb{Q}$ and let $\mc{R}\coloneqq\l\{R_1,R_2\r\}\subseteq\mc{S}$ where $R_1\coloneqq\l(-\infty,b\r)$ and $R_2\coloneqq\l(a,+\infty\r)$. Observe then that $0<\l|\mc{R}\r|<\infty$ and that
$$\begin{equation}
...
Since $\mb{R}$ is unbounded, there are no minimal nor maximal element and hence all basis elements are of the form $\l(a,b\r)$.<span style="float:right;">$\blacksquare$</span>
