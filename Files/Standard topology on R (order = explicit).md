...
_Let $<$ be the standard ordering on the real numbers $\mb{R}$. Then the order topology_ $\mc{T}(<)$ _on $\mb{R}$ is equal to the topology_
$$\begin{equation}
...
_Proof_. We shall show that $\mc{T}_\textrm{st}$ is indeed generated by
$$\begin{equation}
    \mc{B}\coloneqq\l\{B\in\mc{P}\l(\mb{R}\r)\mid\ex a,b\in\mb{R}:a<b\land B=\l(a,b\r)\r\}.
\end{equation}$$
Since $\mb{R}$ has neither a lower nor an upper bound, we see that $\mc{T}_\textrm{st}=\mc{T}\l(\mc{B}\r)=\mc{T}(<)$. To do so, we verify that $\mc{B}\subseteq\mc{T}_\textrm{st}$ and that$$\begin{equation}
