...
_The standard topology $\mc{T}\l(<\r)$ on $\mb{R}$ can be explicitly given as_
$$\begin{equation}
...
\end{equation}$$
Since $\mb{R}$ has neither a lower nor an upper bound, we see that $\mc{T}_\textrm{st}=\mc{T}\l(\mc{B}\r)=\mc{T}(<)$. Firstly, verify the axioms for a topology on $X$.
